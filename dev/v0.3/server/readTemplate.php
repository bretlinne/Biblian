...
if($_POST){
    ...
    if($book->deleteSelectedBooks($bookIds)){
        ...
        $respType = 'success';
        $resp = 'Delete from Database Successful';
...
if($totalRows > 0){
    ...
    while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        echo "<div class='titleAndStatusContainer'>";
            ...
            // ------------------------------------------------------------------------------
            echo "<div class='updateAndStatusContainer'>";
                // ---------------------------
                // READING LIST TOGGLE SWITCH
                // ---------------------------
                echo "<div class='toggleContainer'>"; 
                    echo "<div class='toggleSwitch'>";
                    if ($Reading){
                        echo "<label class='readingListSwitch'><input type='checkbox' checked='true' onclick='readingListSwitchAction(this)'><span class='slider round'></span></label></div>";
                        echo "<div class='toggleStateImageFinished'></div>";
                    }
                    else{
                        echo "<label class='readingListSwitch'><input type='checkbox' onclick='readingListSwitchAction(this)'><span class='slider round'></span></label></div>";
                        echo "<div class='toggleStateImageFaded'></div>";
                    }
                    
                    
                echo"</div>";// END toggleContainer
                ...
                echo "<div class='statContainer'>";
                    // IF FINISHED
                    if ($DateFinished){ 
                        echo "<div class='statusFin' statusValue='finished'>";
                    }
                    // IF NOT FINISHED
                    else{
                        // AND ON READING LIST
                        if ($Reading){
                            echo "<div class='statusReading' statusValue='reading'>";    
                        } else{
                            if ($Progress === 0 || $Progress === null){
                                echo "<div class='statusUnread' statusValue='unread'>";        
                            } else if ($Progress > 0){
                                echo "<div class='statusStarted' statusValue='started'>";
                            } else{
                                echo "<div class='statusFin' statusValue='finished'>";
                            }
                        }
                ...
                echo "<div class='dateFinishedLabel'>{$DateFinished}</div>";
            echo "</div>"; // END statusAndUpdateContainer
            

                // ------------------------------------------------------------
                // OLD VERSION OF STATUS WHEN THEY WERE BUTTONS
                // ------------------------------------------------------------
                // // IF FINISHED
                // if ($DateFinished){ 
        ...
        echo "<div class='advContainer advanced-toggle' onclick='slideToggle(this, 200)'><b>Book Details  <span id='details-icon' class='glyphicon glyphicon-chevron-down'></span></b>";
                echo "<div class='advanced' style='display:none'>Details:";
                    ...
                    echo "<div value={$DateFinished}>Date Finished: {$DateFinished}</div>";
                    echo "<div value={$Progress}>Progress: {$Progress}</div>";
                    echo "<div value={$PageCount}>PageCount: {$PageCount}</div>";
