...
if ($_POST){
    if ($_POST['mode'] === 'addBook') {
        ...
        if($book->updateReading($book->ID, 1)){
            echo "<div class='alert alert-success'>Added to List</div>";   
            $stmt = $book->getBooksOnList($fromRecordNum, $recordsPerPage);
    ...
    if ($_POST['mode'] === 'removeBooks'){
        ...
        // debug_to_console('sizeOf: ' . sizeof($bookIds));
        // debug_to_console('sizeOf: ' . sizeof($titles));
        
        // if on success of delete, reload all books from DB to refresh the UI
        if($book->updateReadingMultiple($bookIds, $titles, 0)){
            echo "<div class='alert alert-success'>Removed book from List</div>";   
        }else{
            echo "<div class='alert alert-success'>Removal Failed!</div>";
        }    
    ...
    if ($_POST['mode'] === 'updateRating'){
        ...
        if($book->updateRating($bookId, $rating)){
            ...
            echo "<div class='alert alert-success'>Rating Update Failed!</div>";   
        }
    }
}
        ...
        echo "<div class='titleAndStatusContainer'>";
            ...
            echo "<div class='statAndUpdateContainer'>";
                ...
                echo "<div class='statContainer'>";
                    ...
                    // if ($Reading){
                    //     //echo "state='on'></div>";
                    //     echo "<label class='finishedSwitch'><input type='checkbox' checked='true' onclick='finishedSwitchAction(this)'><span class='slider round'></span></label>";
                    // }
                    // else{
                    //     //echo "state='off'></div>";
                    //     echo "<label class='finishedSwitch'><input type='checkbox' onclick='finishedSwitchAction(this)'><span class='slider round'></span></label>";
                    // }
