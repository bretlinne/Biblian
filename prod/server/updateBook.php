...
if($_POST['Title']){
            ...
            if ($subject->deleteAllBookSubjectEntries($book->ID)){
                ...
                if($subjectsToAttach !== null && $subjectsToAttach !== []){
                    ...
                    if ($subject->createBookSubject($book->ID, $subjectsToAttach)){
                        // %26 is escape char for '&'
                        $resp = 'Book Updated %26 Book Subject Record Updated';
                        $respType = 'success';
                    }
                }
                if($deleteOnly){
                    $respType = 'success';
                    $resp = 'Successfully removed attached subjects';
                }
                $book->readOne();
            }
        } // END if $changed
    ...
    // --------------------------------------------------------
    if($resp){
        // urlencode() is to help pass special characters in the url parameter like '&'
        $params = "msg=". urlencode($resp)."&type=". urlencode($respType)."&id=".urlencode($book->ID);
        // $params = '';
